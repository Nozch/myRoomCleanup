...
import { renderTrashHistory } from './trash/trashHistoryUi';
import './style.css'
import { setUpTrashUi } from './trash/trashAddUi';
...
function setupInputBlinking() {
  const inputTrashName = document.querySelector<HTMLInputElement>('input[name="trashName"]');
  const labelTrashName = inputTrashName?.closest('label')?.querySelector('.field-label');
  ...
  const inputTrashNote = document.querySelector<HTMLInputElement>('input[name="trashNote"');
  const labelTrashNote = inputTrashNote?.closest('label')?.querySelector('.field-label');
...
function setupSelectBlinking() {
  const select = document.querySelector<HTMLSelectElement>('select[name="trashType"]');
